...
export default function page({params}) {
  ...
  return (
    <main className=" flex flex-col w-screen min-h-screen">
      ...
      { loading && <Spinner size='xl' className=' self-center my-6' color="black" />}
      <section>
        { blogPost.map((post) => (
          <section key={post.id} className=" w-screen h-auto flex flex-1 flex-col sm:items-center md:items-start md:flex-row md:justify-around mx-4 md:mx-12 gap-6 my-12 lg:my-24" >
            <Image src={post.banner_url} alt={post.title} width={500} height={500}  className=" w-full h-1/3 md:w-1/3 md:h-auto rounded-lg"/>
            <div className="  font-Neon space-y-10 w-full md:w-1/2">
              <h2 className=" font-extrabold text-2xl flex flex-row justify-between">{post.title}<span className="flex items-center mx-8 gap-2"><BiSolidUpvote />{post.votes}</span></h2>
              {post.tags && post.tags.map((tag) => (
                <Badge variant='subtle' colorScheme='teal'  key={tag} className="mx-4 my-2">{`#${tag}`}</Badge >
              ))}
              <p className=" font-medium text-xl text-left w-11/12 ">{post.content}</p>
            </div>
          </section>
